...
pub(crate) fn parse(args: impl IntoIterator<Item = String>) -> Result<Cli, CliError> {
...
fn parse_command(args: &[String]) -> Result<Command, CliError> {
    match args {
        ...
        [group, action, rest @ ..] if group == "user-reports" && action == "list" => {
            let options = list_options(rest)?;
            if options.status == "pending_review" {
                return Err(CliError::InvalidArguments);
            }
            Ok(Command::UserReportsList {
                status: options.status,
                cursor: options.cursor,
                limit: options.limit,
            })
        }
        [group, action, id] if group == "user-reports" && action == "inspect" => {
            Ok(Command::UserReportsInspect {
                report_id: identifier(id)?,
            })
        }
        [group, action, id, rest @ ..] if group == "user-reports" && action == "resolve" => {
            let action = required_option(rest, "--action")?;
            if !matches!(action, "reviewed" | "actioned" | "dismissed") {
                return Err(CliError::InvalidArguments);
            }
            Ok(Command::UserReportsResolve {
                report_id: identifier(id)?,
                action: action.to_owned(),
            })
        }
...
pub(crate) const fn usage() -> &'static str {
    "pakperk-admin\n\
     Requires DATABASE_URL, ADMIN_OIDC_ISSUER_URL, ADMIN_OIDC_AUDIENCE,\n\
     ADMIN_AUTHORIZED_USER_IDS, and PAKPERK_ADMIN_ACCESS_TOKEN_FILE in the\n\
     environment. The token file must be absolute, private to its owner, and\n\
     contain a token with an explicit recent auth_time for an existing active\n\
     Pakperk account whose local UUID is in the operator allowlist.\n\
     Commands:\n\
       ...
       user-reports list --status <open|reviewed|actioned|dismissed> [--cursor <cursor>] [--limit <1..100>]\n\
       user-reports inspect <report-id>\n\
       user-reports resolve <report-id> --action <reviewed|actioned|dismissed>\n\
...
mod tests {
    ...
    fn parses_every_supported_command_without_accepting_credentials() {
        ...
        assert!(matches!(
            parse_args(&["user-reports", "list", "--status", "open"])
                .unwrap()
                .command,
            Command::UserReportsList { .. }
        ));
        assert!(matches!(
            parse_args(&["user-reports", "inspect", ID])
                .unwrap()
                .command,
            Command::UserReportsInspect { .. }
        ));
        assert!(matches!(
            parse_args(&["user-reports", "resolve", ID, "--action", "actioned"])
                .unwrap()
                .command,
            Command::UserReportsResolve { .. }
        ));
        assert_eq!(
            parse_args(&["user-reports", "list", "--status", "pending_review"]).unwrap_err(),
    ...
    #[test]
    fn identifiers_and_reason_values_are_explicit_and_strict() {
        assert_eq!(
            parse_args(&[
                "comments",
                "inspect",
