...
void main() {
  test('light and dark themes expose readable semantic surfaces', () {
    ...
    expect(
      dark.extension<PakPerkSemanticColors>(),
      PakPerkSemanticColors.dark,
    );
    expect(
      light.extension<PakPerkSkeletonTheme>(),
      PakPerkSkeletonTheme.light,
    );
    expect(
      dark.extension<PakPerkSkeletonTheme>(),
  ...
  test('interactive component themes retain Material target sizes', () {
    ...
    expect(
      theme.navigationBarTheme.elevation,
      PakPerkElevation.navigation,
    );
    expect(theme.dialogTheme.elevation, PakPerkElevation.modal);
  });

  test('semantic status roles support dark mode, copying, and interpolation',
      () {
    ...
    expect(
        midpoint.processing, Color.lerp(light.processing, dark.processing, .5));
    expect(
        midpoint.moderation, Color.lerp(light.moderation, dark.moderation, .5));
    expect(
