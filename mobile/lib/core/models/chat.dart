...
class ChatEvidence {
  ...
  String get badgeLabel {
    final heading =
        sectionHeading.trim().isEmpty ? sectionKind : sectionHeading.trim();
...
class ChatAnswer {
  ...
  factory ChatAnswer.fromJson(Map<String, dynamic> raw) {
    ...
    return ChatAnswer(
      answerMarkdown:
          (json['answer_markdown'] ?? json['content'] ?? '').toString().trim(),
...
class ChatSnapshot {
  const ChatSnapshot({this.threadId, this.messages = const []});
