...
in the repository. The last full local canonical harness at checkpoint
`d83ee84` passed the fresh-database Rust workspace, Flutter analysis and all 610
then-locked tests, every Android debug and iOS simulator flavor, strict artifact
inspection, 31 browser tests, Helm validation, and the opt-in Collector export
E2E. Subsequent current-tree mobile changes pass Dart formatting, full analysis,
and all 49 tests in the six modified comment suites, while their expanded full
Flutter suite remains a current-source gate. The current public user/developer
guides, curated site packaging, 18-scenario public-edge contract, and hermetic
validators also passed review; browser execution remains scoped to the last
full checkpoint because this runner cannot launch Chromium. Fresh isolated
...
The current tree additionally has fail-closed canonical source contracts for
the five non-restore production approval manifests, their final-render
pre-deploy bundle, sanitized dark-deployment provenance, the protected service
exercise, and the three-gate operational evidence bundle. Their focused
regressions pass, but no production approval manifest, live cluster capture,
staging-smoke artifact, provider/service exercise, migration, live telemetry,
signed-device performance/crash observation, or accountable platform decision
has been produced by this local work; those rows remain **P/H**.
...
schema-v6 artifact requires all 42 ordered scenarios with their exact role
assignments, 317 ordered assertion IDs, 254 closed integer metrics, exact platform
...
run challenge/ID/attempt/time window. Its schema-2 source binding and retained
coordinates also bind the exact staging API, app-link, OIDC issuer, and client
identities. The root-imported candidate and provenance are canonical schema-v4
manifests carrying an identical nested schema-v6 feature binding for the four
To Read First flags, all ten Plan 02 mobile flags, and all ten Plan 03 mobile
flags. The driver request also binds the exact scenario/assertion/metric counts
and canonical contract SHA-256
`7483820afc6b2111f4886177dd120e72ab8ca47164757ca1eda9e10f64d70ad5`.
The v6 markers
cover fresh guest installs and arXiv handoff, offline process-death reading,
cross-device removal/tombstone convergence, complete comment/report replay,
invalid refresh, physical links, signed-device protection, cache bounds, and
light/dark paths. They also require cross-platform queue-authority/FIFO,
fail-closed pending/offline/reset/removal, account-scope rollout revalidation,
and exact/title/error/retry/idempotent Add Paper coverage. Five additional
ordered scenarios cover Plan 02 Search/Lookup/Explore/saved queries, research
profile separation/reset, evidence-backed Why/feedback, reading-brief progress
without queue mutation, and subscription/notification safety. Ten more cover
Deep Reader navigation, Passport, semantic facets, visual-source integrity,
checkpoint isolation, annotation/evidence durability, research memory, version
diff/re-anchor safety, Assistant v2 evidence integrity, and queue-safe large
documents. Its orchestration and validators are repository evidence, not a
passing run. These scenarios do not replace the automated suites or prove every
feature permutation; separate protected staging, privacy, observability,
rollout, and external gates still apply. Only
authenticated root-side provenance/session import and
...
| OIDC JWT validation is strict | R/P | Protected release-issuer rotation/removal: replacement-key and pre-removal old-key tokens are accepted; after the configured cache bound, the same old-key token is rejected with a positive remaining lifetime; independent token expiry/refresh is exercised |
...
| Shared rate limits work across instances | R/P | Protected cross-replica quota exhaustion on one domain-separated bucket/scope commitment with stable HTTP 429, `RATE_LIMITED`, delta-seconds `Retry-After`, and reset evidence |
...
| Feature kill switches are tested | R/P | Repository behavior exists; all 30 independently controlled feature/kill switches still need dark-deployed before/after results, and all 39 required dependency edges need fail-closed rejection evidence. The protected-service exercise remains intentionally scoped to its legacy six; the queue-first discovery and default-off Deep Reader cases require separate full-map migration reconciliation evidence. |
...
| Dark deployment and staging/production parity | Passing canonical `deployment-binding-v1:sha256:` artifact from the repository validator; promotion handoff; live immutable release-binding ConfigMap and values digest; settled actual Pod `imageID` values; chart/version; auth, database-role, NetworkPolicy, feature-map, retention, and secret-version identifiers; reviewed ingress identity/configuration; exact-candidate protected staging smoke results; platform approval | Platform/service/database owners; [deployment provenance runbook](runbooks/release.md#dark-deployment-provenance-evidence) |
| Protected auth, replay, shared-limit, and switch exercise | Passing canonical schema-1 `protected-service-exercise-sha256:` evidence from the repository validator and exact protected workflow; exact candidate and `deployment-binding-v1:` identity; deployment-image, release-configuration, topology, database, service, issuer, application, freshly validated root-owned content-addressed dedicated/ephemeral runner-session, driver, validator, workflow, exact driver-request contract, fresh run/attempt/challenge, and domain-separated `protected-service-subject-sha256:` approval-subject bindings; release issuer/discovery/JWKS identities, configured JWKS cache bound, old/replacement key IDs, accepted replacement-key and pre-removal old-key results, positive remaining old-token lifetime, and removed-key rejection after the bound; same-operation/same-idempotency-identity library and comment replay results with one durable side effect each; at least two independently identified serving replicas, quota/window/action and one domain-separated bucket/scope identity, requests routed through both replicas, stable cross-replica HTTP 429 + `RATE_LIMITED` + delta-seconds `Retry-After`, and reset result; before/after rendered values and observed allowed/disabled behavior for `ACCOUNTS_ENABLED`, `LIBRARY_ENABLED`, `LIBRARY_WRITES_ENABLED`, `COMMENTS_ENABLED`, `COMMENT_CREATION_ENABLED`, and `ACCOUNT_DELETION_ENABLED` under valid dependency combinations, including all six fail-closed invalid-combination results; 29 closed assertions/20 measurements, UTC window, cleanup, and six distinct identity/service/database/platform/release/privacy-safety post-run approvals | Identity/service/database/platform/release owners, plus privacy/safety for authenticated UGC; [protected exercise runbook](runbooks/release.md#protected-auth-write-and-switch-exercise) |
| Public edge and associations | Passing exact-source `public-edge-sha256:` artifact; target environment; promotion-handoff candidate ID; public-site source marker; HTTPS/TLS/HSTS/header/document/runtime/readiness results; a complete gzip public feed with valid trailer integrity, bounded decompression, closed JSON envelope, exact cache policy, and `Vary: Accept-Encoding`; deployed Android/Apple association identities; workflow approval; separately passing `deployment-binding-v1:sha256:` evidence for Pod image IDs, release binding, and reviewed ingress-controller configuration | Platform/release owners; [release runbook](runbooks/release.md#public-edge-technical-evidence), [deployment provenance runbook](runbooks/release.md#dark-deployment-provenance-evidence), and [app-links runbook](mobile-app-links.md) |
| Migration and rollback exercise | Passing canonical `pakperk-migration-exercise-v1:sha256:` manifest and its exact expected binding; source, `deployment-binding-v1:` identity, candidate/configuration/tool and migration-image digests; verified restore evidence ID; starting/ending schema 18/24 and app versions; migration Job status/log digest; DDL-role proof; expand/old-new compatibility/schema-compatible code rollback/re-forward sequence; Plan 03 private-data deletion, restore reapply, and integrity checks; all 30 feature-switch states and fail-closed results for all 39 required dependency edges; exact approval-subject ID, UTC window, cleanup, and database/release approvals. Schema/domain v1 is retained because the canonical shape/hash algorithm is unchanged, but prior schema-10-to-11, schema-11-to-16, schema-11-to-17, six-switch, eleven-switch, or 21-switch evidence, as well as completed schema-11-to-18 evidence, cannot satisfy the current subject, closed assertion, and metric set. | Database and release owners; [operational evidence bundle](runbooks/release.md#operational-gate-evidence-bundle) and [deployment procedure](runbooks/release.md#expandcontract-deployment) |
...
| Live telemetry, retention, and alerts | Passing canonical `pakperk-live-telemetry-v1:sha256:` manifest and exact expected binding; source, production and staging `deployment-binding-v1:` identities, candidate/configuration/tool identities; same candidate images but distinct production and staging-copy Collector/gateway/adapter configuration, redaction, alert-policy, receiver, and retention identities; reviewed parity diff; exact six inputs/19 rules, all 19 routed through owned `page`/`ticket` receivers in both environments, including the privacy-safe confirmed recommendation-card leakage guard; privacy-safe production canary delivered to the production sink; the same bound production retention canaries present initially and during day 29, absent after day 30, with reconciled UTC timestamps/ages/counts; staging valid/hostile mobile probes, protected-sentinel absence, restart replay/coverage, and page/ticket canaries; exact approval-subject ID, cleanup, and platform/observability/privacy approvals | Platform/observability/privacy owners; [operational evidence bundle](runbooks/release.md#operational-gate-evidence-bundle) and [observability runbook](runbooks/observability.md) |
...
| Protected physical mobile acceptance | Passing closed three-member tar containing canonical schema-v6 `mobile-acceptance-evidence.json`, canonical `mobile-acceptance-tooling.json`, and `SHA256SUMS` for the exact root-imported schema-v4 candidate/provenance manifests and source; protected environment approval; short-lived root-owned runner-session attestation; protected validator and driver digests; local tar digest plus immutable upload artifact ID and server digest; fresh run challenge/ID/attempt/time binding; four distinct installation hashes and four challenge-keyed physical-identity hashes derived from distinct root-attested commitments for Android gesture, Android three-button, iPhone home-indicator, and physical-keyboard iPad/second-sync roles; all 42 ordered scenarios with exact role assignments, 317 assertion IDs, 254 closed integer metric rules, and driver-request contract digest `7483820afc6b2111f4886177dd120e72ab8ca47164757ca1eda9e10f64d70ad5`, including the baseline reader/identity/safety/device paths, To Read First, Add Paper, Plan 02 Search/Profile/Why/Brief/Alerts, and ten Plan 03 reader/research scenarios; cached first-readable-frame p95 at or below 1,500 ms and opening transition at or below 700 ms; exact platform artifact/application/signer/team plus schema-2 staging API/app-link/issuer/client bindings and nested schema-v6 feature evidence; sanitized-data markers | Mobile QA/release approval; repository schemas and markers do not replace an exact passing run or automated, visual, accessibility, platform, identity, privacy, observability, and release judgment; [mobile release runbook](mobile-release.md) |
| Mobile performance and crash window | Passing canonical `pakperk-mobile-performance-v1:sha256:` manifest and exact expected binding; source/deployment/candidate/configuration/tool identities; exact signed APK/IPA, version/build and device/OS matrix; at least 20 cold/warm cached first-readable-frame and 20 opening samples, p95 at most 1,500 ms, and opening at most 700 ms; at least 20 sequential cache requests with 95% hits and zero blank cards; at least 20 frame samples; at least a 24-hour window and 200 aggregate exact-candidate sessions from privacy-reviewed source query/store reports from both platforms; at least 99.5% crash-free sessions; exact approval-subject ID, cleanup, and mobile QA/release/privacy approvals | Mobile QA/release/privacy owners; [operational evidence bundle](runbooks/release.md#operational-gate-evidence-bundle) and [mobile release runbook](mobile-release.md#telemetry-and-release-candidate-gates) |
...
gate or its exact `protected-service-exercise-sha256:` artifact. The checked
workflow and validator are only its repository mechanism; no protected
provider/cluster run or owner approval has occurred. Likewise, the restore
harness proves a fail-closed mechanism, not that a provider backup was restored.
