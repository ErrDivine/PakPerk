...
belong under You. Route restoration IDs are stable. The paper reader's Abstract,
Introduction, and Connections stage remains an in-screen interaction with its
existing committed-transition preparation boundary.

## Consequences

- Switching tabs preserves scroll position, nested navigation, and selected
  state instead of rebuilding destinations.
- Routes can support `/`, `/paper/:paperId`, `/paper/:paperId/comments`,
  `/you`, `/you/library`, and settings/account destinations without making
  comments a third primary tab.
